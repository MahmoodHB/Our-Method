...
class TPGAN():
    ...
    def build_generator(self, name="generator", base_filters=64): 
        ...
        c16 = Conv2D(base_filters*4, (3, 3), padding='same', strides=(2, 2), name=name+'_c16', use_bias=False, kernel_initializer=TruncatedNormal(stddev=0.02))(c128r)
    ...
    def build_train_generator_model(self):
        ...
        img128, fc2, front_leye_img, front_reye_img, front_nose_img, front_mouth_img\
        ...
        img128_Consistency = Lambda(lambda x:x, name = "01img128Consistency128")(img128)
		...
		img128_df = Lambda(lambda x:x, name = "04tv")(img128)
        subject_id = Lambda(lambda x:x, name = "09contenet")(subject_id)
        ...
        ret_model = CloudableModel(inputs=[in_img, in_leye, in_reye, in_nose, in_mouth, in_noise],
                          outputs=[img128_px, img128_Consistency128, img128_ip, img128_adv, img128_tv, img128_df, subject_id, leye, reye, nose, mouth],
    ...
    def train_gan(self, gen_datagen_creator, gen_train_batch_size, gen_valid_batch_size,
                  ...
                  optimizer,
                  gen_steps_per_epoch=10, disc_steps_per_epoch=10, epochs=10, 
                  out_dir='../out/', out_period=5, is_output_img=False,
                  lr=0.001, decay=0, lambda_128=1,
                  lambda_Consistency128=0.011, lambda_ip=1e1, lambda_adv=2e1, lambda_tv=1e-3,
                  lambda_contenet=0.05, lambda_parts=3):

        for i in prange(epochs):
            print('train generator {}/{}'.format(i+1, epochs))

            lambda_128_i = lambda_128 
            lambda_Consistency128_i = lambda_Consistency128
            lambda_ip_i = lambda_ip
            lambda_adv_i =lambda_adv
            lambda_tv_i = lambda_tv
			lambda_df_i = lambda_df
            lambda_contenet_i = lambda_contenet
            lambda_parts_i=lambda_parts
            print('params for this epoch\n\
                  lambda_128:{}\n\
                  lambda_Consistency128:{}\n\
                  lambda_ip:{}\n\
                  lambda_adv:{}\n\
                  lambda_tv:{}\n\
				  lambda_df:{}\n\
            ...
            g_history = self.train_generator(train_gen=gen_train_datagen, valid_gen=gen_valid_datagen, optimizer=optimizer, steps_per_epoch=gen_steps_per_epoch,
                                           ...
                                           lr=lr, lambda_128=lambda_128_i,
                                           lambda_Consistency128=lambda_Consistency128_i, lambda_ip=lambda_ip_i, lambda_adv=lambda_adv_i, lambda_df=lambda_df_i, lambda_tv=lambda_tv_i,
                                           lambda_contenet=lambda_contenet_i, lambda_parts=lambda_parts_i)
    ...
    def train_generator(self, train_gen, valid_gen, optimizer=Adam(lr=0.0001), steps_per_epoch=3, epochs=1, is_output_img=False, out_dir='D:/desktop/tpgan_keras/keras_tpgan/out/', out_period=1,
              lr=0.001, decay=0, lambda_128=1,
              lambda_Consistency128=0.011, lambda_ip=1e1, lambda_adv=2e1, lambda_tv=1e-3,
              lambda_contenet=0.05, lambda_parts=3):
        ...
        class SaveSampleImageCallback(Callback):
            ...
            def on_epoch_end(self, epoch, logs):
                ...
                if (epoch + 1) % self.period == 0:
                    ...
                    img128, _, _, _, _, _, _, subject_id, front_leye_img, front_reye_img, front_nose_img, front_mouth_img = self.model.predict(inputs)
                    for i in prange(len(img128)):
        ...
        def _Consistency128(y_true, y_pred):
            """
            Consistency128 loss for size 128x128
            """
            loss_Consistency128 = tf.reduce_mean (np.abs(y_true - y_pred))
			return lambda_128 * Consistency128
        ...
        def contenet(y_true, y_pred): 
            ...
            contenet loss
            """
            return lambda_class * tf.reduce_sum (tf.square(y_true - y_pred))
        ...
        if self.generator_train_model is None:
            ...
            self.generator_train_model.compile(optimizer=optimizer,
                                loss={'00img128px': _loss_img128_px,
                                      '01img128Consistency128': _loss_Consistency128,
                                      '02ip': _loss_ip,
                                      '03adv': _loss_adv,
                                      '04tv': _loss_tv,
                                      '09contenet':_loss_contenet,
                                      '10leye': _loss_part,
                                      '11reye': _loss_part,
                                      '12nose': _loss_part,
                                      '13mouth': _loss_part},
    ...
    def generate(self, inputs):
        ...
        img128_ = (img128*np.iinfo(np.uint8).max).astype(np.uint8)
        img128 = np.array(255*(img128_)**0.5, dtype = 'uint8')
        front_leye_img_ = (front_leye_img*np.iinfo(np.uint8).max).astype(np.uint8)
        front_leye_img = np.array(255*(front_leye_img_)**0.5, dtype = 'uint8')
        front_reye_img_ = (front_reye_img*np.iinfo(np.uint8).max).astype(np.uint8)
        front_reye_img = np.array(255*(front_reye_img_)**0.5, dtype = 'uint8')
        front_nose_img_ = (front_nose_img*np.iinfo(np.uint8).max).astype(np.uint8)
    ...
    def rotate_parts(self, inputs):
        ...
        out_leyes_ = (out_leyes*np.iinfo(np.uint8).max).astype(np.uint8)
        out_leyes = np.array(255*(out_leyes_)**0.5, dtype = 'uint8')
        out_reyes_ = (out_reyes*np.iinfo(np.uint8).max).astype(np.uint8)
        out_reyes = np.array(255*(out_reyes_)**0.5, dtype = 'uint8')
        out_noses_ = (out_noses*np.iinfo(np.uint8).max).astype(np.uint8)
        out_noses = np.array(255*(out_noses_)**0.5, dtype = 'uint8')
        out_mouthes_ = (out_mouthes*np.iinfo(np.uint8).max).astype(np.uint8)
        out_mouthes = np.array(255*(out_mouthes_)**0.5, dtype = 'uint8')
...
class LightCNN():
    ...
    def train(self, train_gen, valid_gen=None, optimizer=SGD(lr=0.001, momentum=0.9, decay=0.00004, nesterov=True),
        ...
        n = self.current_epochs
        np.save("train_model@history__"+str(n),history.history)            
        return history
