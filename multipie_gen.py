...
IMG_H, IMG_W, NUM_SUBJECTS = 128, 128, 346
...
LEYE_Y, LEYE_X , REYE_Y, REYE_X, NOSE_Y, NOSE_X, MOUTH_Y, MOUTH_X = 40, 42, 40, 86, 71, 64, 87, 64
...
class Datagen():
    ...
    def __init__(self, dataset_dir='', landmarks_dict_file='',
                 ...
                 min_angle=-90, max_angle=90, include_frontal=False, 
        ...
        self.datalist_file = os.path.join(datalist_dir)
    ...
    def load_landmarks(self, data_path): 
        ...
        return self.landmarks_dict[subject][landmark]
    ...
    def get_generator(self, batch_size = 16, setting = 'train'):
        ...
        def get_next():
            ...
            for i, x_data_path in enumerate(datalist):
                x_dataset_path = self.dataset_dir + '/side_face'
                x_image_path = os.path.join(x_dataset_path, x_data_path + '.jpg')
                x_image = self.imread(x_image_path, normalize=True)
                
                x_landmarks = self.load_landmarks(x_data_path)

                try:
                    x_face, x_leye, x_reye, x_nose, x_mouth = self.crop(x_image, x_landmarks, 0)
                ...
                y_data_path = x_data_path[:-5]
                y_dataset_path = self.dataset_dir + '/front_face'
                y_image_path = os.path.join(y_dataset_path,y_data_path +'.jpg')
                ...
                y_face128 = resize(y_face, (128, 128), mode='constant')
                ...
                if first_time:
                    ...
                    y_faces128 = y_face128[np.newaxis,:]
                    ...
                    y_faces128 = np.concatenate((y_faces128, y_face128[np.newaxis,:]), axis=0) 
            ...
            return [x_faces, x_leyes, x_reyes, x_noses, x_mouthes, x_z], [y_faces, y_faces, y_faces, y_faces, y_faces, y_subject_ids, y_leyes, y_reyes, y_noses, y_mouthes]        
    ...
    def get_class_generator(self, batch_size = 16, setting = 'train'):
        ...
        def get_next():
            ...
            for i, x_data_path in enumerate(datalist):
                ...
                try:
                    x_face = self.crop(x_image, x_landmarks, 0)[0]
                except (Exception, cv2.error) as e:
                    print(e)
                    print(x_data_path)
                    continue
    ...
    def get_discriminator_generator(self, generator, batch_size=16, gt_shape=(4, 4), setting = 'train'):
        ...
        def get_next():
            ...
            for data_path_for_fake in datalist:
                ...
                x_dataset_path = self.dataset_dir + '/side_face'
                profile_image_path = os.path.join(x_dataset_path, data_path_for_fake + '.jpg')
                profile_image = self.imread(profile_image_path, normalize=True)
                profile_landmarks = self.load_landmarks(data_path_for_fake)
                
                try:
                    profile_face, profile_leye, profile_reye, profile_nose, profile_mouth = self.crop(profile_image, profile_landmarks, 0)
                except (Exception, cv2.error) as e:
                    print(e)
            ...
            for data_path_for_real in datalist:
                ...
                front_data_path = data_path_for_real[:-5]
                y_dataset_path = self.dataset_dir + '/front_face'
                front_image_path = os.path.join(y_dataset_path ,front_data_path +'.jpg') 
